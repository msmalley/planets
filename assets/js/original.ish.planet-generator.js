...
{
    var planet_stats = 'Galactic Coordinates [';
    planet_stats+= unicorn_planet.coordinates[0] + ', ';
    planet_stats+= unicorn_planet.coordinates[1] + ', ';
    planet_stats+= unicorn_planet.coordinates[2] + '] - ';
    planet_stats+= '<b class="biome-key ocean-biome" style="border-color: '+unicorn_planet.oceans[3]+';">Oceans <i class="stat">??</i>%</b> ';
    planet_stats+= '<b class="biome-key rural-biome" style="border-color: '+unicorn_planet.rural[3]+';">Rural <i class="stat">??</i>%</b> ';
    planet_stats+= '<b class="biome-key urban-biome" style="border-color: '+unicorn_planet.urban[3]+';">Urban <i class="stat">??</i>%</b>';
    planet_stats+= ' - <a href="#" class="search-again">Search</a> Again?';
    
    var planet_title = unicorn_planet.name + ' is at WAR!!! You must pick a side - ';
    planet_title+= '<a href="#">' + unicorn_planet.rulers.male + '</a> Vs ';
    planet_title+= '<a href="#">' + unicorn_planet.rulers.female + '</a>';
    $('.planet-name').parent().find('h6').remove();
    $('.planet-name').html(planet_title);
    $('.planet-stats').html(planet_stats);
    $('.planet-stats').after('<h6>'+unicorn_planet.name+' is one of ' + unicorn_planet.total + ' planets stored on the Ethereum blockchain and is currently owned by <a href="#">' + unicorn_planet.owner + '</a></h6>');
...
function generatePlanetStatistics(topology, plates, action)
{
    ...
    var intro = '(both evolved from ' + unicorn_planet.animals.species.toLowerCase() + ' ';
    intro+= 'on a planet with ' + unicorn_planet.oceans[4].toLowerCase() + ' oceans ';
    intro+= 'accompanied by thick ' + unicorn_planet.rural[4].toLowerCase() + ' vegetation and ';
    intro+= 'several ' + unicorn_planet.urban[4].toLowerCase() + ' city states)';
    $('#header .planet-name').after('<h6>' + intro + '</h6>');
